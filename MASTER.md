...
| **Finance** | Credit pane for the AI data-center buildout: claimed ~$5T envelope and ~$4T debt (Tunguz), 70% mid-case $3.5T versus the $4T headline, Columbia / Beignet leverage as primary, debt-service waterfall, SIFMA / Fed stocks with a flow-versus-stock caveat, municipal private-use limits. Every USD figure is Cited or Sample. A labeled Hypergrid bridge (from `commitments.ts` / `datacenters.ts`) shows committed GW, operational (status) GW, and campus headline GW as separate families, never on a USD axis. Economics stays real estate and construction. |
...
### v1.3 — 2026-09-05
- Added **Finance** view: sourced capital stack, debt-service waterfall, credit-market context, municipal channel. Money figures stamp Cited or Sample. A labeled Hypergrid bridge keeps committed GW, operational (status) GW, and campus headline GW on separate families; energized MW and COD stay off the credit charts.
